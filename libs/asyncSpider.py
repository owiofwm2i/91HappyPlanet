...
class Spider(Config):
    ...
    def __init__(self):
        ...
        self.downloader = DownLoader(self.savePath)
    ...
    def getVideoUrlList(self, pageUrl):
        ...
        try:
            ...
            videoTitles = html.xpath(
                "//div[contains(@class,'well well-sm videos-text-align')]/a/span[contains(@class,'video-title title-truncate m-t-5')]/text()")
            videoDurations = html.xpath("//div[contains(@class,'well well-sm videos-text-align')]/a/div/span/text()")
            log.logger.info(
                "videoTitle:{}, videoDuration:{}, videoUrl:{}, videoThumb:{}".format(videoTitles, videoDurations,
                                                                                     videoUrls, videoThumbs))
    ...
    async def getM38UUrl(self, session, url):
        ...
        try:
            ...
            videoDecodeUrl = re.compile(r'src=\'(http[a-z\.:0-9\/]+)').search(
                unquote(videoEncodeUrl, encoding='utf-8', errors='replace')).group(1)
                ...
                async with aiohttp.ClientSession(connector=aiohttp.TCPConnector(verify_ssl=False)) as session:
                    ...
                    if videoDecodeUrl:
                        self.downloader.run(videoDecodeUrl, videoThumbUrl, videoTitle=videoTitle,
                                            videoDuration=videoDuration, videoUrl=videoUrl)
                    else:
                        log.logger.exception(videoDecodeUrl)
                        log.logger.info("任务{}已经存在".format(videoDecodeUrl))
                    log.logger.info(
                        "videoTitle:{}, videoDuration:{}, videoUrl:{}, videoThumb:{}".format(videoTitle, videoDuration,
                                                                                             videoDecodeUrl,
                                                                                             videoThumbUrl))
            except Exception as e:
                log.logger.exception(
                    "videoTitle:{}, videoDuration:{}, videoUrl:{}, videoThumb:{}, exception:{}".format(videoTitle,
                                                                                                       videoDuration,
                                                                                                       videoUrl,
                                                                                                       videoThumbUrl,
                                                                                                       e))
    ...
    def eventLoop(self, pageUrl):
        ...
        q = asyncio.Queue()
        [q.put_nowait((videoTitles[i], videoDurations[i], videoUrls[i], videoThumbs[i])) for i in
         range(len(videoTitles))]
